...
template <typename T>
ListNode<T>::ListNode(T *dat, ListNode<T> *nex) : data(dat),
  next(nex)
{
}   // ListNode()
...
template <typename T>
List<T>& List<T>::operator+= (T *dat)
 {
   ListNode<T> *ptr, *prev = NULL;
   
   for (ptr = head; ptr && *ptr->data < *dat; ptr = ptr->next)
     prev = ptr;
   
    if (!prev)
      head = new ListNode<T>(dat, head);
   else  // if inserting in middle of list.
     prev->next = new ListNode<T>(dat, ptr);
